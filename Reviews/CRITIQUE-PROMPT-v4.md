# Multi-Persona Critique Prompt — v4 (Multi-Dimensional Scoring)
...
**Why v4 differs from v1/v2/v3.** Earlier rounds used coarse verdict scales (acquire / acquire-with-revisions / pass; original / competent / not-for-specialists; faithful / partial / apologetic; star rating). Useful for tracking gross movement, but too lossy to surface what specifically worked and what specifically did not. A revision that genuinely improves five things and worsens one will hold its verdict, masking the signal. The v4 prompt asks each persona for a structured rubric of dimension-level scores with one-sentence reasoning and one-sentence concrete suggestion per dimension where the score is below ceiling — producing an actionable diff list rather than a verdict.

**Other v4 calibration changes:**
- The lay reader prompt no longer pre-loads willingness to be negative. The v3 lay reader rating (3.5) was partly prompt-shaped; the substantive complaints were real but the half-star drag was an artifact of "must be willing to rate three stars or below" framing. v4 asks for honest reaction without nudging toward a critical posture.
- **Reviewers are explicitly instructed to read the entire manuscript end-to-end and review it as a complete published book.** No diff-thinking. No references to what changed in prior revisions, what was cut, or what was added. The review is a holistic verdict on the work as it stands, not an evaluation of any specific revision pass. v4 prompts therefore avoid mentioning prior rounds, threading work, cut appendices, or any other change-history signal that would invite reviewers to read for diffs rather than for the book.
- The reading remit no longer pre-specifies a diagnostic hypothesis. Reviewers read fully and surface their own highest-leverage observations.
- Each reviewer is asked to identify, in priority order, the three changes most likely to move the manuscript up a tier. This converts the round directly into a fix list.
...
- v3 findings & calibration: `Reviews/V3-FINDINGS-2026-04-28.md`
...
1. Save all four reviews plus the synthesis to /Users/mishkinfaustini/workspace/Money, Trust, and Power Book/Reviews/PERSONA-CRITIQUES-{today's date}-v4.md.
...
- **Do not run v4 against the same manuscript state as v3.** Run v4 after a revision pass that responds to v3 findings, so the round actually tests changes rather than re-confirming what is already known.
- **Four reviews remains the right size.** Editor for commercial, historian for scholarly defensibility, heterodox for tradition-faithfulness and policy-implication clarity, lay reader for the actual reading experience. If a fifth read is desired, the highest-value addition is a craft-focused read (a writer-reviewer in the Verlyn Klinkenborg / John McPhee mold) on prose, voice, and chapter-opening hooks — none of the four current personas score line-level prose as their home dimension.
...
- **Half-star ratings and half-point scores are intentional.** The v3 round demonstrated that a binary verdict scale loses signal; half-points are the smallest granularity that distinguishes genuine improvement from noise.
- **Watch for the heterodox / lay reader pull-apart.** v3 surfaced a strategic tension: the heterodox path (sharper forcing argument, restored policy clarity) and the establishment-credible path (steelmanning intact, balance preserved) are not jointly satisfiable. v4 should expose whether any subsequent revision has shifted that trade-off — but the trade-off itself is an author decision, not a manuscript craft fix.
